...
function getRandomFruits(arr){
    let rFruits = []
    for (let i = 0; i<9; i++) {
        rFruits.push(arr[Math.floor(Math.random()*arr.length)])
    }
    return rFruits
}
...
fetch(food)
    .then(res => res.json())
    .then(data => {
        const rFruits = getRandomFruits(makeFruitArray(data))
        slotMachine.innerHTML = rFruits.map(el => `<li>${el.htmlCode}</li>`).join('')
    })
